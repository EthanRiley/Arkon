...
class settings:
    ...
    def load(self, setting_name):
        ...
        npc = this.onscreen_sprites['Lucidia Bright']
        print(npc)
        print(npc.get_imagename())
...
class moveable(sprite):
   ...
   #     if 'sounds' in sorted(kwargs.keys()):
    ...
    def move(self, dx, dy , **kwargs):
        rect = self.get_rect()
        rect.move(dx, dy)
        if this.background.get_rect().collidepoint(rect.x, rect.y):
            for sprite in this.onscreen_sprites:
                if sprite != self._imagename:
                    ...
                    if rect.contains(rect1):
                            ...
                            self.moving = False
                            print("collsion with sprite:  " + sprite)
        ...
        if self.moving:
            if (self.facing == "right") | (self.facing == "left"):
                ...
                if self.facing == "left":
                 ...
                 self.image.blit(self.animation[self.facing][self.__frame], (0, 0))
...
#              'classname' :  self.__class__.__name__ }
    ...
    def activate(self):
        ...
        for sprite in this.onscreen_sprites:
            rect1 = this.onscreen_sprites[sprite].get_rect()
            if ((self.facing == "forward") & (rect1.y >= rect.y - 3) & (rect1.x >= rect.x - 3) & (rect1.x >= rect.x + 3) |
                    (self.facing == "backward") & (rect1.y <= rect.y + 3) & (rect1.x >= rect.x - 3) & (rect1.x >= rect.x + 3)|
                    (self.facing == "left") & (rect1.x >= rect.x - 3) & (rect1.x <= rect.x) & (rect1.y + 3 >= rect.y) & (rect1.y - 3 <= rect.y)|
                    (self.facing == "right") & (rect1.x <= rect.x + 3) & (rect1.x <= rect.x) & (rect1.y + 3 >= rect.y) & (rect1.y - 3 <= rect.y)):
...
class battle_NPC(sprite):
    ...
    def get_persuasion(self):
        return self._entity.get_persuasion(self)
    ...
    def do_attack(self):
        ...
        if stats["determination"] < 25:
            ...
            print(moves)
            self._entity.use_move(moves[random.randrange(0, len(moves))], this.onscreen_sprites["player"])
...
ENEMY PERSUATION: """ + str(this.battle_npc.get_persuasion()["determination"]))
...
def get_name():
    ...
    while iterate:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if char == "space": char =  ' '
            elif char == "backspace": name = re.match(r'(.*).', name).group(1)
            elif char == "return": iterate = False
            else: name+=char
...
THE Z TO ACTIVATE THINGS IN FRONT OF YOU WHATEVER THAT MEANS. WEIRD THAT YOU CAME WITH AN OPERATION MANUAL""", speaker = "Poor Richard")
